...
public class FetchUsersService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        logger.success("FetchUsersService started");
        fetchConnectedUserFriends(localUserSingleton.getUid());
        return START_STICKY;
    }

    private void fetchConnectedUserFriends(String uid) {
        if (uid == null) {
            logger.v("UID is null");
            return;
        }
        firestore.collection(Constants.Firestore.USERS_COLLECTION).document(uid).addSnapshotListener((value, error) -> {
            if (error != null) {
    ...
    private void fetchSingleUser(String uid) {
        String path = SharedPreferencesUtils.getDataByKey(getApplicationContext(), "user_" + uid);
        if (path == null) {
            ...
            fetchedUsersList.add(user);
            usersSubject.onNext(fetchedUsersList);
            logger.success("Retrieved user from SharedPreferences");
        }
    }
