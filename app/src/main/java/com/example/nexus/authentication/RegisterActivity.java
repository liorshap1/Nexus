...
public class RegisterActivity extends AppCompatActivity {
    ...
    protected void insertUserToCollection(User user, Callback callback){
        firebaseFirestore.collection(Constants.Firestore.USERS_COLLECTION).document(user.getUid())
                .set(user)
                .addOnCompleteListener(task -> {
                    if (task.isSuccessful()){
                        localUserSingleton.initializeLocalUser(user.getFirstName(), user.getSecondName(), user.getEmail(), "00", user.getUid(), new ArrayList<>());
